// 041022.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
void print_phonebook() {
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        if (NULL == phonebook[i][0]) {  // нет данных ФИО
            ...
            // оформление - при коротких именах съезжает телефон
// задача: при длине ФИО 8 и менее символов - добавлять 2 \t
...
void fill_demo() {
    ...
    add_phone("Arestovich", "456987");
    add_phone("Bober", "789876");
    add_phone("Pavlovna", "234234");
    add_phone("Vasiliev", "267543");
    add_phone("Kornitskiy", "987345");
    add_phone("Radchenko", "567341");
    add_phone("Pivovarov", "123456");
    add_phone("Xao", "456234");
...
void order_by_name() {
    // принцип сортировки - сравниваем соедние эл-ты и при необходимости переставляем местами
    // особенность - сравнение строк НЕ такое, как у чисел; обмен значениями - как указатели
    bool need_restart;
    do {
        need_restart = false;
        for (size_t i = 0; i < N - 1; i++) {
            ...
            if (phonebook[i][0] != NULL && phonebook[i + 1][0] != NULL) {
                if (strcmp(phonebook[i][0], phonebook[i + 1][0]) == 1) {
                    ...
                    char* tmp_ptr = phonebook[i][0];        // [i] --> "WWW" 
                    phonebook[i][0] = phonebook[i + 1][0];  //  \ "CC"
                    phonebook[i + 1][0] = tmp_ptr;          //  / "WWW"
                    // синхронно с именами нужно менять номера телефонов
...
bool delete_record() {
    ...
    // + перемещение ячеек на освобожденную позицию (не должно быть пропусков)
    ...
    do {
        ...
        index = atoi(user_choice);  // ASCII-to-int ф-ия сканирует строку, если что-то не так возвращаот 0, если все ок - возвращ.число
                                          // мы проверили, правильно ли число ввел пользователь
        if (index <= 0 || index > N) {
    ...
    if (phonebook[i][0] == NULL) {  // ячейка не заполнена
        ...
        std::cout << "Are you syre (delete "
            << phonebook[i][0] << ' ' << phonebook[i][1]
            << ") Y/...";
        std::cin.getline(user_choice, 10);
        // проверить, что ввод у или Y
        if (_stricmp(user_choice, "y") == 0) {  // удаление подтверждено
            ...
            for (size_t j = i; j < N-1; j++) {
                phonebook[j][0] = phonebook[j + 1][0];  // переносим ФИО
                phonebook[j][1] = phonebook[j + 1][1];  // переносим тел.
            }
            phonebook[N - 1][0] = NULL;  // последняя ячейка смещена на предпоследнюю
            phonebook[N - 1][1] = NULL;  // а для последней - значение 0
                                         // иначе будет дублирование последней записи
            return true;
        }
        else {
...
int main()
{
    ...
    do {
        ...
        case '6':
            if (delete_record()) print_phonebook();
            break;
